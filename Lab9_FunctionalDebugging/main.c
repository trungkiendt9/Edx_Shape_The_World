...
int main(void){  unsigned long i,last,now, last_state;
  ...
  i = 0;          // array index
  last = NVIC_ST_CURRENT_R;			// record state time value
  EnableInterrupts();           // enable interrupts for the grader
	last_state = GPIO_PORTF_DATA_R&0x13; // record PF4,1,0
  while(1){
		//if (((GPIO_PORTF_DATA_R&0x11)==0x11) || ((GPIO_PORTF_DATA_R&0x11)==0x00)){
		if (((GPIO_PORTF_DATA_R&0x11)==0x11)){
			GPIO_PORTF_DATA_R &= ~0x02;
		}else {
			GPIO_PORTF_DATA_R ^= 0x02;
		}
    if(i<50 && ((GPIO_PORTF_DATA_R&0x13) != (last_state))){
